	...
	var methods = {
		init : function(options) {
			return this.each(function(){
				//default value
				var options = $.extend({
					icon_path 		: '/_assets/view/icons/picture.png',
					prev_path 		: '/_assets/view/icons/prev.png',
					next_path 		: '/_assets/view/icons/next.png',
				...
				$.getJSON(url, function(data){
					if (data != undefined) {
						...
						$.each(data, function(index, item){
							if (index == 0) {
								//page bar
								page_bar = '<div class="clear"><a class="ly_picture_view_flash" href="' + options.picture_path + '?page_curr=' + item.prev + '"><img src="' + options.prev_path + '" /></a><a class="ly_picture_view_flash" href="' + options.picture_path + '?page_curr=' + item.next + '" ><img src="' + options.next_path + '" /></a> ' + item.curr + ' / ' + item.size + '</div>';
							}
							else {
								//picture
								datas.push('<img style="padding:' + options.img_pad + ';float:left;width:' + options.img_width + ';height:' + options.img_height + ';" src="' + options.file_path + item.fnum + '" file_num="' + item.fnum + '" title="' + item.name + '"  />');
							}
						});
